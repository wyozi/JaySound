...
public class FFT extends FourierTransform
{
  ...
  public void scaleBand(int i, float s)
  {
    if (s < 0)
    {
      System.err.println("Can't scale a frequency band by a negative value.");
      return;
    }
  ...
  public void setBand(int i, float a)
  {
    if (a < 0)
    {
      System.err.println("Can't set a frequency band to a negative value.");
      return;
    }
  ...
  public void forward(float[] buffer)
  {
    ...
    {
      System.err.println("FFT.forward: The length of the passed sample buffer must be equal to timeSize().");
      return;
    }
    doWindow(buffer);
  ...
  public void forward(float[] buffer, int startAt)
  {
	  ...
	  {
		  System.err.println( "FourierTransform.forward: not enough samples in the buffer between " +
  ...
  public void forward(float[] buffReal, float[] buffImag)
  {
    ...
    {
      System.err.println("FFT.forward: The length of the passed buffers must be equal to timeSize().");
      return;
    }
  ...
  public void inverse(float[] buffer)
  {
    ...
    {
      System.err.println("FFT.inverse: the passed array's length must equal FFT.timeSize().");
      return;
    }
    // conjugate
